...
import { deleteCategory } from "@/app/(app)/finance/categories/actions";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function CategoriesTable({ categories: initial }: { categories: Category[] }) {
  ...
  const filtered = React.useMemo(() => {
    ...
    return categories.filter(
      (c) => c.isActive === active && (!q || c.name.toLowerCase().includes(q) || c.description.toLowerCase().includes(q))
    );
  ...
  const onSaved = (category: Category) => {
    const exists = categories.some((c) => c.id === category.id);
    setCategories((prev) =>
      prev.some((c) => c.id === category.id) ? prev.map((c) => (c.id === category.id ? category : c)) : [category, ...prev]
    );
    toast.success(exists ? t("Category updated") : t("Category added"));
  };
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative max-w-sm flex-1">
            ...
            <Input value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} placeholder={t("Search category")} className="ps-9" />
          </div>
          <div className="inline-flex rounded-lg bg-muted p-1 text-sm">
            <SegBtn active={active} onClick={() => { setActive(true); setPage(1); }}>{t("Active Categories")}</SegBtn>
            <SegBtn active={!active} onClick={() => { setActive(false); setPage(1); }}>{t("Inactive Categories")}</SegBtn>
          </div>
          <CategoryDialog onSaved={onSaved}>
            <Button><Plus className="size-4" /> {t("New Category")}</Button>
          </CategoryDialog>
        </div>
              ...
              {paged.length === 0 ? (
                ...
                paged.map((c) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell className="text-muted-foreground"><span dir="auto">{c.description || "—"}</span></TableCell>
                    <TableCell>
                      <div className="flex items-center justify-end gap-1">
                        <CategoryDialog category={c} onSaved={onSaved}>
                          <Button type="button" variant="ghost" size="icon" className="size-8 text-primary" aria-label={t("Edit category")}>
                            <Pencil className="size-4" />
                          </Button>
                        </CategoryDialog>
                        <DeleteCategory name={c.name} onConfirm={() => onDelete(c.id)} />
                      </div>
                    </TableCell>
                  </TableRow>
                ))
...
function DeleteCategory({ name, onConfirm }: { name: string; onConfirm: () => void }) {
  const t = useT();
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Button type="button" variant="ghost" size="icon" className="size-8 text-destructive hover:text-destructive" aria-label={t("Delete category")}>

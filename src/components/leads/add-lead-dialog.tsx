...
export function AddLeadDialog() {
  ...
  const [form, setForm] = React.useState<NewLeadInput>(EMPTY);
  ...
  return (
    ...
    >
      <SheetTrigger asChild>
        <Button>
          <Plus className="size-4" /> {t("Add Lead")}
        </Button>
      </SheetTrigger>
      <SheetContent side="right" className="flex w-full flex-col gap-0 p-0 data-[side=right]:w-full data-[side=right]:sm:max-w-2xl data-[side=right]:lg:max-w-4xl">
        <SheetHeader className="border-b px-6 py-4">
          <SheetTitle>{t("New Lead")}</SheetTitle>
        </SheetHeader>
          ...
          <div className="flex flex-1 flex-col gap-5 overflow-y-auto p-6">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <Field label={t("Phone Number")} required>
                <Input value={form.phone} onChange={(e) => set("phone", e.target.value)} placeholder={t("Enter Phone Number")} inputMode="tel" />
              </Field>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <Field label={t("Phone Number 2")}>
                <Input value={form.phone2} onChange={(e) => set("phone2", e.target.value)} placeholder={t("Enter Phone Number 2")} inputMode="tel" />
              </Field>
              <Field label={t("Email")}>
            ...
            <div className="flex flex-wrap items-center gap-8">
              <div className="flex items-center gap-4">
                ...
                <RadioGroup
                  ...
                  value={form.gender}
                  onValueChange={(v) => set("gender", v as NewLeadInput["gender"])}
                >
                  <div className="flex items-center gap-2">
                    <RadioGroupItem value="male" id="lead-gender-male" />
                    <Label htmlFor="lead-gender-male" className="font-normal">{t("Male")}</Label>
                  </div>
                  <div className="flex items-center gap-2">
                    <RadioGroupItem value="female" id="lead-gender-female" />
                    <Label htmlFor="lead-gender-female" className="font-normal">{t("Female")}</Label>
                  </div>
                </RadioGroup>
              </div>
              ...
              <div className="flex items-center gap-3">
                <span className="text-sm font-medium text-[#595959]">{t("Blocked")}</span>

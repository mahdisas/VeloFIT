-- =============================================================================
-- 00020 — Sold Packages / Sold Products: exclude canceled orders.
--
-- report_sold_items() (00012) counted items from EVERY order, so a canceled
-- order still inflated the "sold" rows and grand total. A canceled order is not
-- a sale — filter it out. Pending orders stay (a sale in progress is a sale).
-- Same signature as 00012 → CREATE OR REPLACE swaps it in place. Idempotent.
-- =============================================================================
...
as $$
  with base as (
    ...
    where oi.gym_id = (select public.auth_gym_id())
      and o.status <> 'canceled'   -- a canceled order is not a sale
